...
async function seedUser () {
  try {
    ...
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Demario30' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'R9flApA8p23t2ri',
      'username':'Cordia15',
      'isDeleted':false,
      'isActive':true,
    ...
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Cordia15' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
